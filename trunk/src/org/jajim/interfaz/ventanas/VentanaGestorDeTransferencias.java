 ...
 * @version 1.2 Supone la interfaz del gestor de transferencias de la aplicación.
 */
public class VentanaGestorDeTransferencias extends JFrame implements ActionListener {
    ...
    private final String sinTransferenciasPendientes = texto.getString(
        "gestor_de_transferencias_sin_transferencias_pendientes");
    ...
    private final JTabbedPane tablonDePestañas;
    private JPanel descargadosPanel;
    private JPanel descargandoPanel;
    private final JLabel etiquetaSinTransferenciasPendientes = new JLabel(sinTransferenciasPendientes);
    private JToolBar barraDeAcciones;
    private final JButton[] botonesBarraDeHerramientas = new JButton[iconos.length];
    ...
    private final List<BarraProgresoSwingWorker> workers;
    ...
    public VentanaGestorDeTransferencias() {
        ...
        Image image = Toolkit.getDefaultToolkit().getImage(ClassLoader.getSystemResource(
    ...
    /**
     * Inicializa la interfaz de la pestaña que se ocupa de manejar los ficheros ya transferidos.;
     */
    ...
    /**
     * Inicializa la interfaz de la pestaña que se ocupa de manejar las transfe rencias pendientes.
     */
    ...
    /**
     * Añade una barra de progreso que especifica como va la transferencia del fi chero.
     * <p>
    ...
    public void añadirTransferenciaDeFichero(String nombre, String idTransferencia, int tipo) {
        ...
        JButton botonCancelar = new JButton(new ImageIcon(ClassLoader.getSystemResource(
            "icons/cancelar_transferencia.png")));
    ...
    /**
     * Elimina la transferencia, cuyo identificador se corresponde con el suminis trado, de la lista de transferencias
     * pendientes del gestor.
     * <p>
    ...
    /**
     * Método que actualiza la interfaz una vez que la transferencia la terminado correctamente. Borra la transferencia
     * del panel de transferencias en descar ga y actuliza la tabla de transferencias descargadas, si el tipo de trans
     * ferencia es de recepción.
     * <p>
     * @param idTransferencia El identificador de la transferencia que se ha de finalizar.
    ...
    public void abortarTransferencias(String contacto) {
        ...
        for (String identificador : identificadores) {
            for (int j = 0; j < workers.size();
                j++) {
                BarraProgresoSwingWorker bpsw = workers.get(j);
                if (identificador.compareTo(bpsw.getIdTransferencia()) == 0) {
                    // Cancelar el hilo
                    bpsw.cancel(true);
                    workers.remove(bpsw);
                    j--;
                    // Cancelar la transferencia
                    tfc.cancelarTransferencia(identificador);
    ...
    /**
     * Método que se ejecuta cuando el usuario selecciona la opción Cerrar del gestor. Oculta la ventana del gestor de
     * transferencias.
     * <p>
